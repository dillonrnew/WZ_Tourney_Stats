...
function Leaderboard({ tournamentId, startingPointsByTeam, startingPointsByRoster }) {
  const [teams, setTeams] = useState([])
  const [mapsByTeamId, setMapsByTeamId] = useState(new Map())
  const [expandedTeamId, setExpandedTeamId] = useState(null)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const resolvedStartingPointsByTeam = startingPointsByTeam ?? EMPTY_STARTING_POINTS
  const resolvedStartingPointsByRoster = startingPointsByRoster ?? EMPTY_STARTING_POINTS
  const startingPointsLookup = useMemo(() => {
    const next = new Map()

    Object.entries(resolvedStartingPointsByTeam).forEach(([teamName, points]) => {
      const normalizedTeamName = normalizeText(teamName)
      const numericPoints = Number(points)
      if (!normalizedTeamName || !Number.isFinite(numericPoints)) return
      next.set(normalizedTeamName, numericPoints)
    })

    return next
  }, [resolvedStartingPointsByTeam])
  const startingRosterPointsLookup = useMemo(() => {
    const next = new Map()

    Object.entries(resolvedStartingPointsByRoster).forEach(([rosterKey, points]) => {
      const normalizedRosterKey = normalizeRosterKey(rosterKey)
      const numericPoints = Number(points)
      if (!normalizedRosterKey || !Number.isFinite(numericPoints)) return
      next.set(normalizedRosterKey, numericPoints)
    })

    return next
  }, [resolvedStartingPointsByRoster])
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const nextTeams = safeTeamRows
          .map((team) => {
            const teamName = team.updated_team_name || 'Unknown Team'
            const rosterPlayers = [team.updated_player_1, team.updated_player_2, team.updated_player_3].filter(Boolean)
            const rosterKey = normalizeRosterKey(rosterPlayers.join(' | '))
            const startingPoints =
              startingPointsLookup.get(normalizeText(teamName)) ||
              startingRosterPointsLookup.get(rosterKey) ||
              0
            ...
            return {
              ...
              score: (scoreByTeamId.get(team.team_id) || 0) + startingPoints,
            }
          })
  ...
  }, [tournamentId, startingPointsLookup, startingRosterPointsLookup])
